...
QVariant LayerItemModel::data(const QModelIndex &index, int role) const {
    if (index.isValid()) {
        ...
        if (role == Qt::DisplayRole || role == Qt::EditRole) {
            ...
            case 5: return QString{"%1, %2, %3"}.arg(data.cubeEdgeLength.x).arg(data.cubeEdgeLength.y).arg(data.cubeEdgeLength.z);
...
LayerDialogWidget::LayerDialogWidget(QWidget *parent) : DialogVisibilityNotify(PREFERENCES_WIDGET, parent), colorDialog(this) {
    ...
    QObject::connect(&dupLayerButton, &QToolButton::clicked, [this](){
        for (const auto & mindex : treeView.selectionModel()->selectedRows()) {
            ...
            state->viewer->resizeTexEdgeLength(Dataset::current().cubeEdgeLength.x, state->M, Dataset::datasets.size());// update layerRenderSettings and textures//FIXME
...
void LayerDialogWidget::updateLayerProperties() {
    ...
    if (currentIndex.isValid()) {
        ...
        combineSlicesSpin.setRange(0, Dataset::datasets[ordered_index].cubeEdgeLength.x * 0.5 * (state->M - 1));//FIXME
