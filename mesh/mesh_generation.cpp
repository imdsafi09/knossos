...
auto generateMeshForSubobjectID(const std::unordered_map<std::uint64_t, std::uint64_t> & soid2oid, const std::vector<std::uint64_t> & objects, const Loader::Worker::SnappyCache & cubes, QProgressDialog & progress) {
    ...
    const auto processCube = [&](const auto & val){
        ...
        const std::size_t size = cubeEdgeLen.z * cubeEdgeLen.y * cubeEdgeLen.x;
        ...
        const auto inMagCoord = Coordinate{pair.first.x, pair.first.y, pair.first.z}.componentMul(cubeEdgeLen);
        const auto cubeCoord = scale.componentMul(inMagCoord);

        const std::array<double, 3> dims{{cubeEdgeLen.x * 1.0, cubeEdgeLen.y * 1.0, cubeEdgeLen.z * 1.0}};
        ...
        for (std::size_t i = 0; i < 6; ++i) {
            const std::array<double, 3> dims{{i < 2 ? 2.0 : cubeEdgeLen.x + 2, i % 4 < 2 ? cubeEdgeLen.y + 2 : 2.0, i < 4 ? cubeEdgeLen.z + 2 : 2.0}};
            ...
            for (std::size_t z = 0; z < dims[2]; ++z) {
                for (std::size_t y = 0; y < dims[1]; ++y) {
                    for (std::size_t x = 0; x < dims[0]; ++x) {
                        ...
                        const CoordOfCube coord(globalPos.x / cubeEdgeLen.x, globalPos.y / cubeEdgeLen.y, globalPos.z / cubeEdgeLen.z);
                        const CoordInCube inCube(globalPos.x % cubeEdgeLen.x, globalPos.y % cubeEdgeLen.y, globalPos.z % cubeEdgeLen.z);
                        auto & cube = extractedCubeForCoord(coord);
                        data[z * sliceSize + y * rowSize + x] = boost::multi_array_ref<uint64_t, 3>(reinterpret_cast<std::uint64_t *>(cube.data()), boost::extents[cubeEdgeLen.z][cubeEdgeLen.y][cubeEdgeLen.x])[inCube.z][inCube.y][inCube.x];
                    }
                }
            }
